...
export function GameArea({ settings, onUpdateStats, allTimeBestStreak, allTimeBestOpm }: GameAreaProps) {
  const { state, startGame, submitAnswer, endGame, resetGame, calculateStats } = useGameState(settings);
  const { playCorrect, playWrong, playStreak, playKeypress } = useGameSounds(settings.soundEnabled);
  
  const [feedbackState, setFeedbackState] = useState<'idle' | 'correct' | 'wrong'>('idle');
  const [lastResult, setLastResult] = useState<{ isCorrect: boolean; answer: number } | null>(null);
  const [showFeedback, setShowFeedback] = useState(false);
  const [showResults, setShowResults] = useState(false);
  const [streakAnimate, setStreakAnimate] = useState(false);

  const hasEnabledOperation = (Object.keys(settings.operations) as OperationType[])
    .some(op => settings.operations[op].enabled);

  const handleSubmit = useCallback((answer: number) => {
    const result = submitAnswer(answer);
    if (!result) return;

    setLastResult({ isCorrect: result.isCorrect, answer: result.question.answer });
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-8">
        <div className="text-center space-y-4">
          <h2 className="text-4xl md:text-5xl font-bold text-highlight">
            Treine sua aritmética mental
          </h2>
          <p className="text-xl text-dim max-w-md mx-auto">
            Resolva operações matemáticas o mais rápido possível. 
            Configure as operações e intervalos nas configurações.
          </p>
        </div>
